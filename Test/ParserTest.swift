...
struct SecretTestSuite {
    ...
    @Test("Test Secret parsing - environment variable interpolation in file path")
...
struct BlkioDeviceRateTestSuite {
    ...
    func parseEnvInterpolation() throws {
        ...
        """
        let node = try Yams.compose(yaml: yaml)
        let rate = try Service.BlkioDeviceRate(node!, envs: ["DEVICE_PATH": "/dev/sda"])
        #expect(rate.path == "/dev/sda")
    }
...
struct DeployTestSuite {
    ...
    func parseFullDeploy() throws {
        ...
        #expect(deploy.resources?.reservations?.devices?.first?.capabilities == ["gpu"])
        #expect(deploy.resources?.reservations?.devices?.first?.driver == "nvidia")
        #expect(deploy.resources?.reservations?.devices?.first?.count == "1")
        #expect(deploy.resources?.reservations?.devices?.first?.device_ids == ["0", "1"])
...
struct HealthcheckTestSuite {
    ...
    @Test("Test Healthcheck parsing - test as single string wrapped as CMD-SHELL")
    ...
    @Test("Test Healthcheck parsing - bare string 'NONE' is wrapped, not disabled")
    ...
    func parseEnvInterpolation() throws {
        ...
        let hc = try Service.Healthcheck(
            node!,
            envs: ["HEALTHCHECK_CMD": "curl -f http://localhost", "INTERVAL": "30s"]
        )
...
struct ServiceBuildTestSuite {
    ...
    func parseEnvInterpolation() throws {
        ...
        let node = try Yams.compose(yaml: yaml)
        let build = try Service.Build(node!, envs: ["BUILD_CONTEXT": "./backend"])
        #expect(build.context == "./backend")
    }
...
struct ServiceConfigTestSuite {
    ...
    func parseEnvInterpolation() throws {
        ...
        let node = try Yams.compose(yaml: yaml)
        let config = try Service.Config(node!, envs: ["CONFIG_NAME": "my_config"])
        #expect(config.source == "my_config")
    }
...
struct ServiceDependencyTestSuite {
    ...
    @Test("Test Dependency parsing - invalid condition value yields nil condition")
...
struct ServiceExtendsTestSuite {
    ...
    func parseAllFields() throws {
        ...
        """
        let node = try Yams.compose(yaml: yaml)
        let extends = try Service.ServiceExtends(node!, envs: [:])
    ...
    func parseEnvInterpolation() throws {
        let yaml = "service: ${BASE_SERVICE}"
        let node = try Yams.compose(yaml: yaml)
        let extends = try Service.ServiceExtends(
            node!,
    ...
    func parseMissingService() throws {
        ...
        #expect(throws: (any Error).self) {
            try Service.ServiceExtends(node!, envs: [:])
        }
    }
...
struct ServiceGpusTestSuite {
    ...
    @Test("Test GPU parsing - string other than 'all' is treated as device list, throws")
...
struct EnvFileEntryTestSuite {
    ...
    func parseEnvInterpolation() throws {
        ...
        let node = try Yams.compose(yaml: yaml)
        let entry = try Service.EnvFileEntry(node!, envs: ["ENV_DIR": "./config"])
    ...
    @Test("Test EnvFileEntry parsing - missing required in long syntax throws")
...
struct ServiceTestSuite {
    ...
    func parseDependsOnArray() throws {
        ...
        let node = try Yams.compose(yaml: yaml)
        let service = try Service(node!, envs: [:])
        #expect(Set(service.depends_on?.map(\.key) ?? []) == Set(["db", "cache"]))
        #expect(service.depends_on?["db"] != nil)
    ...
    func parseNetworksArray() throws {
        ...
        let node = try Yams.compose(yaml: yaml)
        let service = try Service(node!, envs: [:])
        #expect(Set(service.networks?.map(\.key) ?? []) == Set(["frontend", "backend"]))
    }
    ...
    func parseExtraHostsList() throws {
        ...
        let node = try Yams.compose(yaml: yaml)
        let service = try Service(node!, envs: [:])
        #expect(service.extra_hosts?.contains("somehost:162.242.195.82") == true)
    }
