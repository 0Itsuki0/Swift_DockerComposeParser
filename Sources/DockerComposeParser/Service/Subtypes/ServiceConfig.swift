...
extension Service.Config {
    func merge(with other: Service.Config) -> Service.Config {
        let old = self
        let new = other
...
extension Array where Element == Service.Config {
    func merge(with otherVolumes: [Service.Config]) -> [Service.Config] {
        ...
        guard let source = try mapping.value(for: CodingKeys.source).string(envs: envs) else {
            throw DecodingError.dataCorrupted(
                .init(
                    ...
                    debugDescription: "Config entry must have a 'source' specified."
                )
            )
        }
        self.source = source
